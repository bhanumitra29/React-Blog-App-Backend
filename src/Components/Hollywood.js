...
const Hollywood = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch("https://nodeserver-api-hl14.onrender.com/api/hollywood");
      if (!response.ok) {
  ...
  <h2 className='headingLatest'>The Latest</h2>
       ...
       <hr className="custom-hr"/>
        ...
        {data.filter((item) => item.id >25 && item.id<29).map((item,index)=>{
     return(
       // <div>
    ...
    <div className='techparent'>
      <div className='leftdiv'>
       ...
       <hr className="custom-hr"/>
        ...
        {data.filter((item) => item.id >=15 && item.id<=21).map((item,index)=>{
          return(
            <div key={index}>
...
{data.filter((item) => item.id ===22).map((item,index)=>{
  return(
    <div key={index}>
      ...
      {data.filter((item) => item.id >22 && item.id<=25).map((item,index)=>{
          return(
            <div key={index}>
            <div className='innerparent1'>
            <Link to={`/dynamic/${item.cat}/${item.id}`} state={{ referrer: '/hollywood' }}>
