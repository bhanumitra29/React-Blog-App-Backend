...
const Technology = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch("https://nodeserver-api-hl14.onrender.com/api/technology");
      if (!response.ok) {
  ...
  <h2 className='headingLatest'>The Latest</h2>
       ...
       <hr className="custom-hr"/>
        ...
        {data.filter((item) => item.id >7 && item.id<11).map((item,index)=>{
     return(
       // <div>
...
{data.filter((item) => item.id ===11).map((item,index)=>{
  return(
    <div key={index}>
      ...
      {data.filter((item) => item.id >11 && item.id<=14).map((item,index)=>{
          return(
            <div key={index}>
            <div className='innerparent1'>
            <Link to={`/dynamic/${item.cat}/${item.id}`} state={{ referrer: '/technology' }}>
